...
def test_kavach_blocks_jagran_until_deployment_confirmed(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(ip, "_STATE_PATH", state_path)
    monkeypatch.setattr(
        ip,
        "_load_jagran_params",
        lambda: {"enabled": True, "dedup_window_seconds": 0, "still_failing_interval_seconds": 0},
    )
    ...
    state_path.write_text(json.dumps({"deployment": {"confirmed": True}}), encoding="utf-8")
    asyncio.run(
        ip.publish_incident(
            source="kavach",
            scenario="order_rejection",
