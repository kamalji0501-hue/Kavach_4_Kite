    ...
    if t == "/ato_report":
        ...
        _bot(
            ...
            (
                ...
                f"Buy : {last['buy_timestamp_ist']} @ Nifty {last['buy_nifty_ltp']}"
                f" / prem {last.get('buy_option_premium', '-')}\n"
                f"Sell: {last['sell_timestamp_ist']} @ Nifty {last['sell_nifty_ltp']}"
                f" / prem {last.get('sell_option_premium', '-')}\n"
                f"Points lost: {last['points_lost']:.2f} (x lots: {last['points_lost_x_lots']:.2f})\n"
                f"Premium P&L: {last.get('premium_pnl', 0):.2f}"
                f" (₹{last.get('premium_pnl_rupees', 0):.2f})"
            ),
        )
        return
...
def _record_sim_ato_exit(
    *,
    ...
    row = {
        ...
        "buy_option_premium": round(float(entry.get("buy_option_premium") or 0.0), 2),
        "sell_timestamp_ist": _sim_time.strftime("%Y-%m-%d %H:%M:%S IST"),
        "sell_nifty_ltp": round(float(spot), 2),
        "sell_option_premium": round(float(entry.get("sell_option_premium") or 0.0), 2),
        ...
        "premium_pnl": round(float(entry.get("premium_pnl") or 0.0), 2),
        "premium_pnl_rupees": round(float(entry.get("premium_pnl_rupees") or 0.0), 2),
