...
START_ORDER = ("drishti", "kavach", "jagran")
...
def run_start_all(
    *,
    ...
    log.info(
        "Step 2: Sequential launch DRISHTI -> KAVACH -> JAGRAN"
    ...
    if not wait_bot_running("drishti", timeout_seconds=wait_seconds * 0.5, root=ROOT):
        log.error("DRISHTI did not reach RUNNING before KAVACH launch.")
    ...
    if not ltp_ok:
        log.warning("Proceeding to KAVACH — LTP gate not satisfied (may be off-hours).")

    _launch_bot("kavach", force=force or True)
    log.info("Gate: waiting for KAVACH RUNNING...")
    if not wait_bot_running("kavach", timeout_seconds=60.0, root=ROOT):
        log.warning("KAVACH not RUNNING yet — JAGRAN launch continues after short delay.")
