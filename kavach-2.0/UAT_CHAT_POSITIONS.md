...
1. Read screenshot → write uat/deployed_positions/positions.json (source=cursor_chat, **8 legs: 6 BUY + 2 SELL**)
...
4. Reply: OK + expiry + 8-leg table + "Tap Register in KAVACH (pick Core PE BUY then Core CE BUY)"
...
1. Read attached Sensibull New Strategy screenshot:
   - Weekly expiry (any date — e.g. 28 Jul 2026)
   - **8 legs**: 6 BUY + 2 SELL (1 PE SELL + 1 CE SELL + multiple PE/CE BUY candidates)
   - B/S, strike, CE/PE, lots, avg price for each
   - Do **not** pre-mark Core BUY — operator picks Core PE BUY / Core CE BUY in Register
...
5. Reply: OK: <full path>\positions.json | expiry=... | 8 legs summary table
...
```
UAT POSITIONS. Read the Sensibull screenshot (note expiry + 8 legs: 6 BUY + 2 SELL) and write positions.json for UAT Register.
