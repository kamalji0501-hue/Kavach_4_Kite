...
1. Buy Lightsail/EC2 Ubuntu in **ap-south-1** with **static IP**
2. Whitelist IP with broker (Breeze / Kite / etc.)
3. Save SSH `.pem` locally — never commit to git
4. Deploy **DEV Batman Algo** to `~/batman-algo/`
5. Configure `.env` — broker JWT, Telegram, strategy params
6. systemd timer — 11:00 entry, 15:15 exit IST
7. Wire Telegram on every trade and error
8. Automate JWT/session token refresh
9. Paper trade / small size first
10. Tell agent new IP + key path — point at this file
