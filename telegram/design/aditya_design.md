# ADITYA Design (Morning Hedge Exit Spec)

Status: DESIGN_PARTIALLY_LOCKED
Last Updated: 2026-05-14
Owner Bot Domain: KAVACH 2.0 -> ADITYA
...
> **Rename note (2026-07-25):** Formerly **PRABHAT MUKTI**. All new docs/code refer to this module as **ADITYA**.


## 1) Scope and Intent

RATRIPAL buys overnight hedges at ~15:20 IST and writes a handoff file.
ADITYA reads that file the next morning, confirms with the user, and
...
(spot outside the short strikes), ADITYA must first buy ATO protection
...
No new Telegram bot is introduced. The 3-bot rule (DRISHTI / KAVACH 2.0 / LAKSHMI)
stays intact.

| Component     | File                                                 | Role                                    |
| ------------- | ---------------------------------------------------- | --------------------------------------- |
| Logic module  | `modules/aditya.py`                           | All decision and execution logic        |
| Bot prompts   | `bat_telegram/bots/kavach2/bot.py`                    | All user-facing confirm/deny/time-pick  |
| Handoff input | `data/analytics/hedge_box/aditya_handoff.csv` | Written by RATRIPAL, read+updated by PM |
| P&L output    | `data/analytics/aditya/aditya_trade_log.csv`      | Written by PM after sell execution      |
| Config        | `config/settings.json` — `aditya` block       | All tunables, no hardcoding             |
| Events        | `core/event_bus.py` — 2 new events                   | Sell prompt request + execution done    |

Reason: KAVACH 2.0 already owns all trading HITL (Hedge Box confirm/deny, emergency exit confirm).
ADITYA sell is a trading action; KAVACH 2.0 is the natural owner of the user-facing flow.
...
## 4) ADITYA Module Schedule (Locked)
...
[❌ Cancel all]
```
Six options, selection-only (no free text). Config key: `aditya.sell_time_options`.
...
- P&L row → `data/analytics/aditya/aditya_trade_log.csv`
...
After ATO buy is verified, ADITYA notifies KAVACH immediately:
```
...
File: `data/analytics/aditya/aditya_trade_log.csv`

| Field           | Description                                              |
| --------------- | -------------------------------------------------------- |
| `buy_date`      | Date RATRIPAL bought the hedge                           |
| `sell_date`     | Date ADITYA sold the hedge                        |
| `side`          | CE or PE                                                 |
...
PM-Q4 — BATMAN COMPLETE BEFORE 9 AM
  ...
  open positions. Should ADITYA still sell them?
...
PM-G10 — STATE FLAG COORDINATION WITH ATO MODULE
  After ADITYA buys ATO in a gap scenario, should it set
...
## 10) What is NOT ADITYA's Responsibility
