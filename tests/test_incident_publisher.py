...
def test_incident_ledger_writes_daily_csv_and_xlsx(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(ip, "_incident_ledger_dir", lambda: ledger_dir)
...
def test_send_to_jagran_retries_after_rate_limit(monkeypatch, tmp_path) -> None:
    ...
    token.write_text("JAGRAN_BOT_TOKEN=token\nJAGRAN_CHAT_ID=12345\n", encoding="utf-8")
    monkeypatch.setattr(ip, "_jagran_token_path", lambda: token)
    ...
    async def _fake_retry(fn, **kwargs):
        # Mimic send_with_retry: call once, on RetryAfter call again
        try:
            return await fn(**kwargs)
        except RetryAfter:
            return await fn(**kwargs)

    monkeypatch.setattr(ip, "send_with_retry", _fake_retry)
