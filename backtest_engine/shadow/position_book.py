...
def apply_virtual_fill(
    ...
    *,
    ...
    if df.empty or sym_col not in df.columns:
        return pd.DataFrame([_resolve_broker_row(symbol, delta, avg_price, expiry_date=expiry_date)])
    ...
    new_row = _resolve_broker_row(symbol, delta, avg_price, expiry_date=expiry_date)
