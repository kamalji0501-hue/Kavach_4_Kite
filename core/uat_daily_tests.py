...
TEST_CATALOG: tuple[TestCaseDef, ...] = (
    ...
    TestCaseDef("UAT-D24", "book", "Instrument master resolves all 4 legs"),
    ...
    TestCaseDef("UAT-D27", "ltp", "Option FNO marketfeed LTP (4 legs)", market_hours_required=True),
...
def run_case_dynamic_expiry() -> CaseResult:
    ...
    try:
        ...
        ok = len(legs) == 4 and exp >= date.today() - timedelta(days=14)
...
def run_case_cursor_chat_source() -> CaseResult:
    ...
    try:
        ...
        ok = source in ("cursor_chat", "sensibull_ocr", "sensibull_test") and len(fix.get("legs", [])) == 4
        return CaseResult("UAT-D30", ok, f"source={source!r} legs=4")
    except Exception as exc:
