...
from bat_telegram.bots.kavach.register_wizard import (  # noqa: E402
...
_CB_DONE = "kav_done"  # batman_complete confirm
...
_CB_MENU = "kav_menu"  # main alive menu buttons
...
def _main_menu_keyboard() -> InlineKeyboardMarkup:
    """Compact inline menu — home screen after /start and unknown messages."""
    from core.feed_recovery import (
        LABEL_AUTO_RESUME,
        LABEL_MANUAL_HANDLING,
        kavach_resume_button_spec,
        show_recovery_control_buttons,
    )

    pause_reason = _read_algo_pause_reason()
    try:
        resume_label, resume_action = kavach_resume_button_spec(pause_reason)
    except Exception:
        resume_label, resume_action = "Resume", "resume"
    try:
        show_recovery = show_recovery_control_buttons(pause_reason)
    except Exception:
        show_recovery = False
    rows: list[list[InlineKeyboardButton]] = [
        [
            InlineKeyboardButton("Register", callback_data=f"{_CB_MENU}:register"),
        ...
        [
            ...
            InlineKeyboardButton(
                "ATO Configuration", callback_data=f"{_CB_MENU}:ato_tune"
            ),
        ],
        [
        ...
        [
            InlineKeyboardButton("Environment", callback_data=f"{_CB_MENU}:environment"),
            InlineKeyboardButton("Funds", callback_data=f"{_CB_MENU}:funds"),
        ],
    ]
    if show_recovery:
        rows.append(
            [
                InlineKeyboardButton(
                    ...
                    callback_data=f"{_CB_MENU}:recovery_operator",
                ),
                InlineKeyboardButton(
                    f"🤖 {LABEL_AUTO_RESUME}",
                    callback_data=f"{_CB_MENU}:recovery_auto",
                ),
            ]
        )
    rows.extend(
        [
            [
                InlineKeyboardButton("Pause", callback_data=f"{_CB_MENU}:pause"),
                InlineKeyboardButton(
                    resume_label, callback_data=f"{_CB_MENU}:{resume_action}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "Start Algo Now", callback_data=f"{_CB_MENU}:start_algo"
                ),
            ],
            [
                InlineKeyboardButton(
                    "Batman Complete", callback_data=f"{_CB_MENU}:batman_complete"
                ),
            ],
        ]
    )
...
async def _send_alive_menu(message: Message) -> None:
    """KAVACH alive card with timestamp and action buttons."""
...
def _append_log(event: str, **kwargs: Any) -> None:
    ...
    try:
        audit_log = _daily_audit_dir() / "kavach_audit.jsonl"
...
def apply_ato_buffer_patch(
    ...
    *,
    ...
    """
    from bat_telegram.bots.kavach.ato_configuration_wizard import apply_patches_dict
...
def _retrace_keyboard() -> InlineKeyboardMarkup:
    """Step 5/6 — retrace_points picker keyboard (11 options: 0–50 in steps of 5).

    These are the ATO exit buffer points: how many points NIFTY must pull back
    past the sell strike before the ATO protection position is exited (prevents whipsaw).
    """
    options = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
    mid = len(options) // 2  # split into 2 rows for mobile readability
    row1 = [
        InlineKeyboardButton(f"{v} pts", callback_data=f"{_CB_RETRACE}:{v}") for v in options[:mid]
    ]
    row2 = [
        InlineKeyboardButton(f"{v} pts", callback_data=f"{_CB_RETRACE}:{v}") for v in options[mid:]
    ]
...
def _buffer_keyboard(same_value: int | None = None) -> InlineKeyboardMarkup:
    """Entry-buffer picker keyboard (0-20 points, selection-only)."""
    options = [0, 5, 10, 15, 20]
    row = [InlineKeyboardButton(f"{v} pts", callback_data=f"{_CB_BUF}:{v}") for v in options]
    rows = [row]
    if same_value is not None:
        rows.append(
            [
                InlineKeyboardButton(
                    f"Use same as CE ({same_value} pts)",
...
async def _wizard_goto_ce_buffer(
    ...
    await _wizard_edit_step(
        ...
        r"Choose the CE entry buffer used before CE ATO fires\.\n"
        r"Allowed values: 0, 5, 10, 15, 20 points\.",
...
def _selected_header(selected: dict[str, dict]) -> str:
    """Build the '✅ PE BUY: ...' confirmation header for wizard steps.

    Locked display order: PE BUY → PE SELL → CE BUY → CE SELL (CONTEXT.md 2026-04-05).
    """
...
def _set_register_conversation_state(
    ...
    key = _register_conversation_key(update)
    if key is not None:
        context.user_data[key] = state_id
...
async def _wizard_fetch_step1(
    ...
    *,
    ...
    pe_body = "Enable *PE side* coverage?"
    plan = wizard_data.get("wiz_plan") or []
    idx, total = question_index(plan, "pe_intent")
    pe_prompt = (
        f"{intro}\n\n" f"*Question {idx} of {total} — PE side — Enable PE side*\n\n{pe_body}"
    )
    ...
    await _wizard_show(
        ...
        reply_target,
        pe_prompt,
        reply_markup=pe_intent_keyboard(),
        prefer_edit=prefer_edit,
    )
    return WIZARD_PE_INTENT
...
async def wizard_ce_buffer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        rf"CE buffer selected: *{ce_val} pts*\n\n"
...
async def wizard_pe_buffer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        rf"*Step 7/{_WIZARD_STEPS_TOTAL} — CE Retrace Points*\n\n"
        r"Choose the CE retrace points used to exit CE ATO after a pullback\.",
        parse_mode=ParseMode.MARKDOWN_V2,
...
async def wizard_ce_retrace(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        rf"*Step 8/{_WIZARD_STEPS_TOTAL} — PE Retrace Points*\n\n"
        rf"CE retrace selected: *{ce_retrace_points} pts*\n\n"
        r"Choose the PE retrace points used to exit PE ATO after a bounce\.",
        parse_mode=ParseMode.MARKDOWN_V2,
        reply_markup=InlineKeyboardMarkup(
            [
                *(_retrace_keyboard().inline_keyboard[:-1]),
                [
                    InlineKeyboardButton(
                        f"Use same as CE ({ce_retrace_points} pts)",
...
async def _wizard_show_summary(query: CallbackQuery, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    summary += (
        f"*Question {cidx} of {ctotal} — Shared — Confirm deployment*\n\n"
    ...
    if wizard_data.get("pe_enabled"):
        ...
        pe_mode = str(wizard_data.get("pe_protect_strike_mode", "AUTO")).lower()
        summary += (
            f"`PE BUY  : {_fmt_leg(selected['pe_buy'])}`\n"
            f"`PE SELL : {_fmt_leg(pe_sell)}`\n"
            f"Managed lots: *{_md2(str(wizard_data.get('pe_managed_lots')))}*\n"
            f"ATO lots on breach: *{_md2(str(wizard_data.get('pe_ato_lots', '—')))}*\n"
            f"ATO PE: `{_md2_code(pe_ato_sym)}` \\({_md2(pe_mode)}\\)\n\n"
        )
    else:
    ...
    if wizard_data.get("ce_enabled"):
        ...
        ce_mode = str(wizard_data.get("ce_protect_strike_mode", "AUTO")).lower()
        summary += (
            f"`CE BUY  : {_fmt_leg(selected['ce_buy'])}`\n"
            f"`CE SELL : {_fmt_leg(ce_sell)}`\n"
            f"Managed lots: *{_md2(str(wizard_data.get('ce_managed_lots')))}*\n"
            f"ATO lots on breach: *{_md2(str(wizard_data.get('ce_ato_lots', '—')))}*\n"
            f"ATO CE: `{_md2_code(ce_ato_sym)}` \\({_md2(ce_mode)}\\)\n\n"
        )
    else:
        summary += f"`{_md2_code('CE side: not registered')}`\n\n"

    summary += f"{_md2('ATO thresholds:')}\n"
    if wizard_data.get("ce_enabled") and "ce_trigger" in levels:
        summary += (
            f"`CE trigger : {_md2_code(f'{levels['ce_trigger']:,}')}  "
            f"\\({_md2(buffer_display(ce_entry_raw))}\\)`\n"
            f"`CE retrace : {_md2_code(f'{levels['ce_retrace']:,}')}  "
            f"\\({_md2(buffer_display(ce_exit_raw))}\\)`\n"
        )
    if wizard_data.get("pe_enabled") and "pe_trigger" in levels:
        summary += (
            f"`PE trigger : {_md2_code(f'{levels['pe_trigger']:,}')}  "
            f"\\({_md2(buffer_display(pe_entry_raw))}\\)`\n"
            f"`PE retrace : {_md2_code(f'{levels['pe_retrace']:,}')}  "
            f"\\({_md2(buffer_display(pe_exit_raw))}\\)`\n"
        )
...
async def wizard_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if wizard_data.get("ce_enabled"):
        confirm_lines.append(
            f"CE buffers: *{_md2(buffer_display(ce_entry_raw))}* / "
            f"*{_md2(buffer_display(ce_exit_raw))}*"
        )
    if wizard_data.get("pe_enabled"):
        confirm_lines.append(
            f"PE buffers: *{_md2(buffer_display(pe_entry_raw))}* / "
            f"*{_md2(buffer_display(pe_exit_raw))}*"
        )
    poll_label = _md2(str(poll_interval_seconds or "config default"))
...
async def cmd_ato_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        with open(dep_file, encoding="utf-8") as fh:
            dep = json.load(fh)
        ce_sell_strike = dep.get("positions", {}).get("ce_sell", {}).get("strike", "N/A")
        pe_sell_strike = dep.get("positions", {}).get("pe_sell", {}).get("strike", "N/A")
    except Exception:
        pass
    sides_label = {
        "pe": "PE side only \U0001f53b",
    ...
    cached = get_cached_nifty_ltp(max_age_seconds=consumer_max_age_for_trading())
    if cached is not None:
        snap = read_nifty_ltp_cache()
        src = snap.source if snap else "cache"
        spot_line = f"\nNIFTY Spot : ₹{_md2(f'{cached:,.2f}')} \\({_md2(src)}\\)"
    else:
        spot_line = f"\nNIFTY Spot : {_md2('cache stale — check DRISHTI feed')}"

    def _fmt_strike(v) -> str:
        return f"{v:,}" if isinstance(v, (int, float)) else str(v)

    ce_trigger_level = (
        int(ce_sell_strike) + int(ce_entry_buffer)
        if isinstance(ce_sell_strike, (int, float))
        else ce_sell_strike
    )
    pe_trigger_level = (
        int(pe_sell_strike) - int(pe_entry_buffer)
        if isinstance(pe_sell_strike, (int, float))
        else pe_sell_strike
    )
    ce_exit_level = (
        int(ce_sell_strike) - int(ce_retrace_points)
        if isinstance(ce_sell_strike, (int, float))
        else ce_sell_strike
    )
    pe_exit_level = (
        int(pe_sell_strike) + int(pe_retrace_points)
        if isinstance(pe_sell_strike, (int, float))
        else pe_sell_strike
    )
    ...
    from bat_telegram.bots.kavach.ato_configuration_wizard import _CB_ATC
    ...
    status_markup = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "ATO Configuration", callback_data=f"{_CB_ATC}:start"
                )
            ],
    ...
    await _reply_md2(
        message,
        f"🛡 *ATO Status*{spot_line}\n\n"
        f"Schedule   : {schedule_line}\n\n"
        f"CE side : {'🔴 TRIGGERED' if ce_trig else '🟢 IDLE'}\n"
        f"  Symbol : `{_md2_code(ce_sym)}`\n"
        f"  Fires ≥: `{_md2_code(_fmt_strike(ce_trigger_level))}`\n"
        f"  Retrace exit ≤: `{_md2_code(_fmt_strike(ce_exit_level))}`\n\n"
        f"PE side : {'🔴 TRIGGERED' if pe_trig else '🟢 IDLE'}\n"
        f"  Symbol : `{_md2_code(pe_sym)}`\n"
        f"  Fires ≤: `{_md2_code(_fmt_strike(pe_trigger_level))}`\n"
        f"  Retrace exit ≥: `{_md2_code(_fmt_strike(pe_exit_level))}`\n\n"
        f"⏱ Poll interval: *{poll_label} sec*\n"
        f"🤖 Algo manages: *{_md2(sides_label.get(managed, managed))}*",
...
async def cmd_pause(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state:
        ...
        state.set("algo.pause_reason", "kavach_manual")
        state.set("algo.paused_by", "kavach")
    bus = context.bot_data.get("event_bus")
    if bus:
        from core.event_bus import Event
...
async def cmd_resume(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not allowed:
        await message.reply_text(
            note, parse_mode=ParseMode.HTML, reply_markup=_main_menu_keyboard()
        )
        return
...
async def cmd_positions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Read-only broker positions (Gate 3 smoke)."""
    message = _require_message(update)
    broker = context.bot_data.get("broker")
    if not broker:
        await _reply_md2(
    ...
    state = context.bot_data.get("state")
    ce_ato_sym = str(state.get("ato.ce_protect_symbol", "") if state else "")
    pe_ato_sym = str(state.get("ato.pe_protect_symbol", "") if state else "")
    ce_active = bool(state.get("ato.ce_triggered", False)) if state else False
    pe_active = bool(state.get("ato.pe_triggered", False)) if state else False

    lines = ["📊 *Open NIFTY Positions*", ""]
    for pos in positions:
        sym = pos.get("display_symbol") or pos["symbol"]
        px = _format_position_price(pos)
        tag = ""
        raw_sym = str(pos.get("symbol", ""))
        if ce_active and raw_sym == ce_ato_sym:
            tag = " ATO"
        elif pe_active and raw_sym == pe_ato_sym:
            tag = " ATO"
        plain = f"{sym}{tag} | {pos['direction']} {pos['qty']} @ {px}"
    ...
    def _on_ato_ce(event, payload):
        _fire(
            _push(
                ...
                chat_id,
                "🔴 *ATO CE TRIGGERED*\n\n"
                "NIFTY has breached the CE sell strike\\.\n"
                "ATO protection buy order placed\\.",
            )
        )
    ...
    def _on_ato_pe(event, payload):
        _fire(
            _push(
                ...
                chat_id,
                "🔴 *ATO PE TRIGGERED*\n\n"
                "NIFTY has breached the PE sell strike\\.\n"
                "ATO protection buy order placed\\.",
            )
        )

    def _on_ato_ce_exited(event, payload):
        _fire(_push(app, chat_id, "🟢 *ATO CE exited* — NIFTY retraced behind CE sell strike\\."))

    def _on_ato_pe_exited(event, payload):
        _fire(_push(app, chat_id, "🟢 *ATO PE exited* — NIFTY retraced behind PE sell strike\\."))
        ...
        if payload.get("soft_cap"):
            ...
            _fire(
                _push(
                    app,
                    chat_id,
                    f"⚠️ *ATO {side} choppy session* \\(exposure {exposure}\\)\n\n"
                    "Soft cap warning — ATO continues\\. Review if needed\\.",
                )
            )
            return
        _fire(
